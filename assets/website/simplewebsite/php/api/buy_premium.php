...
$serviceKind = 1; // adjust if needed
...
<body>
  ...
  <p id="premiumExpiry">
    <?php if ($expireDate): ?>
      Your current premium expires on: <strong><?= htmlspecialchars($expireDate) ?></strong>
    <?php else: ?>
  ...
  <script>
    ...
    const premiumExpiryElem = document.getElementById('premiumExpiry');
    ...
    buyButton.addEventListener('click', async () => {
      ...
      const totalMs = day * 86400000 + hour * 3600000 + min * 60000;
      const newExpireDate = new Date(baseDate.getTime() + totalMs);

      // Format as YYYY-MM-DD HH:mm:ss
      function pad(num) { return num.toString().padStart(2, '0'); }
      const expireDateStr = `${newExpireDate.getFullYear()}-${pad(newExpireDate.getMonth() + 1)}-${pad(newExpireDate.getDate())} ${pad(newExpireDate.getHours())}:${pad(newExpireDate.getMinutes())}:${pad(newExpireDate.getSeconds())}`;

      try {
        const response = await fetch('/php/api/buy_premium.php', {
          method: 'POST',
        ...
        if (result.success) {
          ...
          // Update currentExpireDate and UI text
          currentExpireDate.setTime(newExpireDate.getTime());
          premiumExpiryElem.innerHTML = 'Your current premium expires on: <strong>' + result.expireDate + '</strong>';
        } else {
